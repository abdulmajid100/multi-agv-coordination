...
class GridEnv:
    ...
    def step(self, actions):
        ...
        return self._get_state(), rewards, done, {'steps': self.steps}

    def _get_state(self):
        """Create a 1D representation of the grid state including agent positions."""
        state = np.zeros(self.grid_size, dtype=int)
        for agent in self.agents:
            state[agent] = 1  # Mark the agent's position
        for obs in self.obstacles:
            state[obs] = -1  # Mark obstacles
        for goal in self.goals:
            state[goal] = 2  # Mark goals
        return state.flatten()  # Return the state as a flattened array
        ...
        def update(frame):
            if frame == 0:
                ...
                for i, agent in enumerate(self.agents):
                    if isinstance(agent, (list, np.ndarray)) and len(agent) > 1:
                        x, y = tuple(agent)  # Unpack the array into x and y coordinates
                    else:
                        x, y = agent, 0  # Handle the case where it's a single value
                ...
                for i, agent in enumerate(self.agents):
                    x, y = tuple(agent) if isinstance(agent, (list, np.ndarray)) else agent  # Ensure it's a tuple
                    agents_patches[i].center = (x, y)

...
# Action representation: a vector where each element is 0 (wait) or 1 (move) for each AGV
...
def train_agents(num_agents, num_episodes, state_size, action_size, fixed_paths):
    ...
    for episode in range(num_episodes):
        ...
        state_matrix = np.zeros((num_agents, state_size))
        for agent_index, path in enumerate(agv_paths):
            if path:
                for node in path:
                    state_matrix[agent_index, node - 1] += 1
        #print(state_matrix)
        state_vectors = [state_matrix[agent_index] for agent_index in range(num_agents)]
        #print(state_vectors)
        for step in range(200):  # Limit the number of steps per episode
            # Update the state matrix based on the current positions

            #print(state_vectors)
            actions, step_log_probs = select_actions(agents, state_vectors)
            #print(actions)
            for agent_index, action in enumerate(actions):
                ...
                log_probs[agent_index].append(step_log_probs[agent_index])
                rewards[agent_index].append(reward)
                states[agent_index].append(state_vectors[agent_index])
...
action_size = 7  # Number of possible actions (one for each node)

# Train the agents
agents_paths, G = train_agents(num_agents, num_episodes, state_size, action_size, fixed_paths)

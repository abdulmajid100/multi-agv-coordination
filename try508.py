...
def can_move(agv, current_node, next_node):
    shared_nodes = [n for n in agv_paths[agv] if
...
# Create animation
ani = animation.FuncAnimation(fig, update, frames=range(1, len(max(agv_paths.values(), key=len))), repeat=False, interval=500)

...
def start_backtracking(agv, conflict_node, waiting_agvs, agv_tasks, resource_states, agv_history, backtracked_agvs):
    ...
    if len(agv_history[agv]) > 1:
        ...
        previous_node = agv_history[agv][-2]
...
def backtrack_further(agv, agv_tasks, resource_states, agv_history, backtracked_agvs):
    ...
    if len(agv_history[agv]) > 1:
        current_node = agv_history[agv][-1]
        previous_node = agv_history[agv][-2]
...
def check_backtracked_agv_resume(backtracked_agvs, resource_states):
    ...
    for agv, info in backtracked_agvs.items():
        ...
        if resource_states[original_conflict_node] != 0:
...
def simulate_digital_twin():
    ...
    # NEW: Track safe sequences for each AGV (excludes moves that lead to deadlocks)
    agv_safe_sequences = {agv: [] for agv in agv_tasks.keys()}
    agv_conflict_points = {}  # Track where each AGV's problems started
    ...
    for agv, tasks in agv_tasks.items():
        if tasks and tasks[0]:
            ...
            agv_history[agv].append(starting_node)
    ...
    while any(tasks for tasks in agv_tasks.values()) and iteration < max_iterations:
        ...
        print(f"AGVs in conflict mode: {list(agv_conflict_points.keys())}")
        ...
        if backtracked_agvs:
            ...
            for agv in agvs_to_resume:
                ...
                # IMPORTANT: Clear conflict point - AGV can add to safe sequence again
                if agv in agv_conflict_points:
                    print(f"Clearing conflict point for {agv} - can now add moves to safe sequence")
                    del agv_conflict_points[agv]
        ...
        for agv, tasks in agv_tasks.items():
            ...
            if tasks and len(tasks[0]) >= 1:
                ...
                if can_move(agv, shared_nodes_with_others, other_agvs, current_node, next_node, resource_states):
                    ...
                    # CRITICAL: Only add to safe sequence if AGV is NOT in conflict mode
                    if agv not in agv_conflict_points:
                        agv_safe_sequences[agv].append((agv, current_node, next_node, 'move'))
                        print(f"SAFE: {agv} moves from {current_node} to {next_node}")
                    else:
                        print(
                            f"CONFLICT MODE: {agv} moves from {current_node} to {next_node} (not added to safe sequence)")
                    ...
                    # Don't add waits to safe sequence during conflict mode
                    if agv not in agv_conflict_points:
                        agv_safe_sequences[agv].append((agv, current_node, current_node, 'wait'))
                        print(f"SAFE: {agv} waiting at {current_node}")
                    else:
                        print(f"CONFLICT MODE: {agv} waiting at {current_node} (not added to safe sequence)")
        ...
        if waiting_agvs or backtracked_agvs:
            ...
            if conflicts:
                # CRITICAL: Mark conflict points for AGVs involved in deadlocks BEFORE resolving
                for conflict_node, conflicting_agvs in conflicts.items():
                    for agv in conflicting_agvs:
                        if agv not in agv_conflict_points:
                            # Find where this AGV started having problems
                            current_safe_length = len(agv_safe_sequences[agv])
                            agv_conflict_points[agv] = {
                                'conflict_node': conflict_node,
                                'safe_sequence_length_before_conflict': current_safe_length,
                                'conflict_started_at_node': agv_history[agv][-1] if agv_history[agv] else None
                            }
                            print(
                                f"MARKING CONFLICT: {agv} at node {agv_history[agv][-1] if agv_history[agv] else 'unknown'} for conflict at {conflict_node}")
                            print(f"  Safe sequence length before conflict: {current_safe_length}")
    ...
    # Combine all safe sequences into final conflict-free sequence
    print(f"\n=== GENERATING FINAL CONFLICT-FREE SEQUENCE ===")
    final_conflict_free_sequences = []

    # Merge all AGV safe sequences in chronological order
    all_safe_moves = []
    for agv in agv_tasks.keys():
        for i, move in enumerate(agv_safe_sequences[agv]):
            all_safe_moves.append((move, agv, i))  # (move_tuple, agv, sequence_index)

    # Sort by the order they would have been added (simple approach: by AGV priority and sequence index)
    agv_priority = {'AGV1': 0, 'AGV2': 1, 'AGV3': 2}
    current_indices = {agv: 0 for agv in agv_tasks.keys()}

    # Interleave moves from different AGVs to maintain chronological order
    while any(current_indices[agv] < len(agv_safe_sequences[agv]) for agv in agv_tasks.keys()):
        for agv in ['AGV1', 'AGV2', 'AGV3']:  # Process in order
            if current_indices[agv] < len(agv_safe_sequences[agv]):
                move = agv_safe_sequences[agv][current_indices[agv]]
                final_conflict_free_sequences.append(move)
                current_indices[agv] += 1

                # Only add one move per iteration to maintain proper ordering
                break

    print(f"Safe moves per AGV:")
    for agv in agv_tasks.keys():
        print(f"  {agv}: {len(agv_safe_sequences[agv])} moves")

    print(f"Total conflict-free moves: {len(final_conflict_free_sequences)}")
    print(f"AGVs that had conflicts: {list(agv_conflict_points.keys())}")

    return final_conflict_free_sequences

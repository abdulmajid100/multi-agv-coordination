...
class AGVSimulation:
    ...
    def generate_random_tasks(self):
        ...
        max_attempts = 100  # Maximum number of attempts to generate feasible paths

        for attempt in range(max_attempts):
            # Reset tasks and resource states
            self.agv_tasks = {}
            self.resource_states = {node: 0 for node in self.graph.nodes()}
            self.total_paths = 0

            # Generate paths for each AGV
            ...
            # Check if first moves are feasible for all AGVs
            all_first_moves_feasible = True

            for agv, tasks in self.agv_tasks.items():
                if len(tasks[0]) <= 1:  # Skip if task has only one node
                    continue
                ...
                for other_agv in other_agvs:
                    if self.agv_tasks[other_agv]:
                        ...
                        shared_nodes = set(tasks[0]) & set(other_current_task)
                        shared_nodes_with_others[other_agv] = list(shared_nodes)

                if not self.can_move_original(agv, shared_nodes_with_others, other_agvs, current_node, next_node):
                    all_first_moves_feasible = False
                    if self.verbose:
                        print(f"First move for {agv} from {current_node} to {next_node} is not feasible. Regenerating paths...")
                    break

            if all_first_moves_feasible:
                if self.verbose and attempt > 0:
                    print(f"Found feasible paths after {attempt+1} attempts")
                return

        if self.verbose:
            print(f"Warning: Could not find feasible paths after {max_attempts} attempts. Using last generated paths.")

    def can_move_original(self, agv, shared_nodes_with_others, other_agvs, current_node, next_node):
        """Original version of the function to check if an AGV can move to the next node."""
        # If next_node is not shared with any other AGV, allow the move
        if all(next_node not in shared_nodes_with_others[other_agv] for other_agv in other_agvs):
            return True

        # If any other AGV is already occupying a shared node, don't allow the move
        if any(
        ...
        for other_agv, shared_nodes in shared_nodes_with_others.items():
            for node in shared_nodes:
                if node not in node_sharing_count:
                    node_sharing_count[node] = 1  # Start with 1 (current AGV)
                node_sharing_count[node] += 1

        # Find nodes shared by 3 or more AGVs
        critical_shared_nodes = [node for node, count in node_sharing_count.items() if count >= 3]

        if critical_shared_nodes and next_node in critical_shared_nodes:
            # We have a node shared by 3+ AGVs, need to determine if this AGV is the "furthest"
            # Calculate distances from current positions to the shared node
            distances = {}
            current_agv_distance = len(self.agv_tasks[agv][0]) - self.agv_tasks[agv][0].index(next_node)
            distances[agv] = current_agv_distance

            for other_agv in other_agvs:
                if self.agv_tasks[other_agv] and next_node in self.agv_tasks[other_agv][0]:
                    other_distance = len(self.agv_tasks[other_agv][0]) - self.agv_tasks[other_agv][0].index(next_node)
                    distances[other_agv] = other_distance

            # If this AGV is the furthest from the shared node, don't allow it to move
            if distances[agv] >= max(distances.values()):
                if self.verbose:
                    print(f"{agv} is furthest from shared node {next_node} in a 3+ AGV junction, preventing deadlock")
                return False
    ...
    def agv_process_original(self, agv):
        ...
        while self.agv_tasks[agv]:
            ...
            self.total_moves += 1
            if self.can_move_original(agv, shared_nodes_with_others, other_agvs, current_node, next_node):
                # Execute move
...
def compare_processes(max_time=1000, verbose=True, simulation_verbose=False):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(original_results['agv_counts'], original_results['avg_success_rates'], 'o-', label='DRR')
    plt.plot(new_results['agv_counts'], new_results['avg_success_rates'], 's-', label='ODRR')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(original_results['agv_counts'], original_results['avg_completion_rates'], 'o-', label='DRR')
    plt.plot(new_results['agv_counts'], new_results['avg_completion_rates'], 's-', label='ODRR')
...
def demonstrate_same_paths():
    """Demonstrate the paths used by both process types and check for deadlocks."""
    print("\n=== Demonstrating paths and checking for deadlocks ===")
    ...
    sim1 = AGVSimulation(env1, G, num_agvs=5, verbose=True, use_original_process=True)
    sim1.generate_random_tasks()

    # Create second simulation with new process
    env2 = simpy.Environment()
    sim2 = AGVSimulation(env2, G, num_agvs=5, verbose=True, use_original_process=False)
    sim2.generate_random_tasks()

    # Show paths for both processes
    ...
    # Check for deadlocks in first moves
    print("\nChecking for deadlocks in first moves:")

    print("\nOriginal process first moves feasibility:")
    check_first_moves_feasibility(sim1)

    print("\nNew process first moves feasibility:")
    check_first_moves_feasibility(sim2)

    # Reset the random seed
    random.seed()

def check_first_moves_feasibility(simulation):
    """Check if the first moves are feasible for all AGVs."""
    all_first_moves_feasible = True

    for agv, tasks in simulation.agv_tasks.items():
        if len(tasks[0]) <= 1:  # Skip if task has only one node
            print(f"{agv}: No move needed (task has only one node)")
            continue
        ...
        # Check if move is possible
        other_agvs = [other_agv for other_agv in simulation.agv_tasks if other_agv != agv]
        shared_nodes_with_others = {other_agv: [] for other_agv in other_agvs}

        for other_agv in other_agvs:
            if simulation.agv_tasks[other_agv]:
                other_current_task = simulation.agv_tasks[other_agv][0]
                shared_nodes = set(tasks[0]) & set(other_current_task)
                shared_nodes_with_others[other_agv] = list(shared_nodes)

        # Use the appropriate movement function based on which process is being used
        if simulation.use_original_process:
            move_feasible = simulation.can_move_original(agv, shared_nodes_with_others, other_agvs, current_node, next_node)
        else:
            move_feasible = simulation.can_move(agv, shared_nodes_with_others, other_agvs, current_node, next_node)

        print(f"{agv}: Move from {current_node} to {next_node} is {'feasible' if move_feasible else 'NOT feasible'}")

        if not move_feasible:
            all_first_moves_feasible = False

    print(f"\nOverall: {'All first moves are feasible' if all_first_moves_feasible else 'Some first moves are NOT feasible'}")
    return all_first_moves_feasible

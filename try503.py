...
for agv, path in agv_paths.items():
    ...
    print([resource_states[shared_node] for shared_node in shared_nodes])
...
def can_move(agv, current_node, next_node):
    # Condition 1: The next node is not in any other AGV's path and is idle
...
def update(frame):
    ...
    for agv, path in agv_paths.items():
        ...
        if current_index < len(path) - 1:
            ...
            shared_nodes1 = [n for n in agv_paths[agv] if
                             n in sum(agv_paths.values(), []) and (sum(agv_paths.values(), []).count(n) > 1)]
            print(shared_nodes1)
            print(agv)
            print(current_node)
            print(next_node)
            print([resource_states[shared_node] for shared_node in shared_nodes1])
...
# Create animation
ani = animation.FuncAnimation(fig, update, frames=range(15), repeat=False, interval=1000)
